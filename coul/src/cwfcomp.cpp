...
// step_num : std::complex step from debut to z. It is (z - debut)/N_num .
...
{ 
  ...
  for (unsigned int i = N_num-1 ; i <= N_num ; i--)
  {
    ...
    if (!isfinite (H) || !isfinite (dH)) std::cout<<"Numerical failure encountered in H_dH_direct_integration."<<std::endl,exit (1);
...
// Calculation of H[omega],H'[omega] for std::complex l with the expansion formula.
// ---------------------------------------------------------------------------
...
// All functions are real, so double values are returned, taking the real part of std::complex variables.
//
// Variables
// ---------
...
// Re[z] < 0 is not considered, as G(z) is std::complex with Im[z] = Im[l] = Im[eta] = 0.
...
{ 
  ...
  if (!is_it_successful)
  {
    ...
    { 
      //// Replace the following line by : if (!neg_int_omega_one && !neg_int_omega_minus_one) H_dH_with_expansion (omega,z,H,dH,is_it_successful);
      //// if you want H_dH_with_expansion to be used if |l_i| < 1 or |z| > 1.
      if (!neg_int_omega_one && !neg_int_omega_minus_one && (std::abs (l_i) >= 1.0) && (abs (z) <= 1.0)) H_dH_with_expansion (omega,z,H,dH,is_it_successful);
		       ...
		       class Coulomb_wave_functions &cwf_lp1)
{
  ...
  if ((cwf_lp1.is_it_normalized != is_it_normalized) || (cwf_lp1.l != l + 1) || (cwf_lp1.eta != eta)) 
    std::cout<<"Parameters of cwf_l and cwf_lp1 are not correct."<<std::endl, exit (1);
  ...
  const std::complex<double> Cl_eta_ratio = exp (log_Cl_eta_calc (l,eta) - log_Cl_eta_calc (l+1,eta)), F_ratio =  Flp1/Fl, H_omega_ratio = H_omega_lp1/H_omega_l;
				  ...
				  std::complex<double> Gz_tab[],std::complex<double> dGz_tab[])
{
  ...
  const std::complex<double> I_omega(0,omega),G_factor_turn = (!is_it_normalized) ? (I_omega*exp (2.0*log_Cl_eta_calc (l_deb + il_turn,eta))) : (I_omega);
...
{  
  const std::complex<double> Ieta(-imag (eta),real (eta)), l_deb_p1_p_Ieta = 1+l_deb+Ieta, l_deb_p1_m_Ieta = 1+l_deb-Ieta;
  if ((l_deb == rint (real (l_deb))) && (real (l_deb) < 0)) std::cout<<"l_deb cannot be a negative integer."<<std::endl, exit (1);
  if ((l_deb_p1_p_Ieta == rint (real (l_deb_p1_p_Ieta))) && (real (l_deb_p1_p_Ieta) < 0)) std::cout<<"1+l_deb+i.eta cannot be a negative integer."<<std::endl, exit (1);
  if ((l_deb_p1_m_Ieta == rint (real (l_deb_p1_m_Ieta))) && (real (l_deb_p1_m_Ieta) < 0)) std::cout<<"1+l_deb-i.eta cannot be a negative integer."<<std::endl, exit (1);
